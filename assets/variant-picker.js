if (!customElements.get("variant-picker")) {
  class VariantPicker extends HTMLElement {
    ...
    init() {
      ...
      const { productId, sectionId, productUrl, hasOnlyDefaultVariant, showFeaturedMedia, variantTradeData, isTradeCustomer } = this.dataset;
      ...
      this.variantTradeData = JSON.parse(variantTradeData)
      this.isTradeCustomer = isTradeCustomer === 'true'
      ...
      this.updateForTradeOnlyProduct();
    }
    ...
    updatePrice() {
      ...
      let { compare_at_price, price, unit_price_measurement, available } = this.currentVariant;
      const variantTradePrice = this.variantTradeData?.find(({ id }) => Number(id) === this.currentVariant.id)?.tradePrice
      const finalPrice = this.isTradeCustomer && variantTradePrice ? Number(variantTradePrice) * 100 : price
      const saleBadgeType = priceWrapper.dataset.saleBadgeType;
      const variantTradeCompareAtPrice = this.variantTradeData?.find(({ id }) => Number(id) === this.currentVariant.id)?.tradeCompareAtPrice
      if (variantTradeCompareAtPrice) compare_at_price = Number(variantTradeCompareAtPrice) * 100;

      const onSale = compare_at_price && compare_at_price > finalPrice;
      ...
      if (salePrice) salePrice.innerHTML = formatMoney(finalPrice, moneyFormat);

      if (compareAtPrice) {
        compareAtPrice.forEach((item) => {
          const displayPrice = onSale ? compare_at_price : finalPrice;
      ...
      if (saleBadge && onSale && saleBadgeType !== "text") {
        ...
        if (saleBadgeType === "fixed_amount") {
          value = formatMoney(compare_at_price - finalPrice, moneyFormat);
        } else {
          const saving = Math.round(((compare_at_price - finalPrice) * 100) / compare_at_price) + "%";
          value = saving;
        }
    ...
    updateForTradeOnlyProduct () {
      const productFormWrapper = document.querySelector(".main-product__block-buy_buttons")
      
      if (!productFormWrapper) {
        // If not ready, try again next tick
        setTimeout(() => {
          this.updateForTradeOnlyProduct();
        })
        return;
      ...
      const variantIsTradeOnly = this.variantTradeData?.find(({ id }) => id === this.currentVariant.id)?.tradeOnly === true
      const priceWrapper = this.productWrapper.querySelector(".m-price");
      const productForm = productFormWrapper?.querySelector(".m-product-form");
      const tradeSignInButton = productFormWrapper?.querySelector(".trade-sign-in-button");

      if (!this.isTradeCustomer && variantIsTradeOnly) {
        // Hide product form + price, show trade signup
        productForm?.classList.add('hidden')
        priceWrapper?.classList.add('hidden')
        tradeSignInButton?.classList.remove('hidden')
      } else {
        // Hide trade signup, show product form + price
        tradeSignInButton?.classList.add('hidden')
