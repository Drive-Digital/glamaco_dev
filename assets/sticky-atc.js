  ...
  customElements.define(
    ...
    class StickyAtc extends HTMLElement {
      ...
      init() {
        ...
        const { variantTradeData, isTradeCustomer, initialSelectedVariant } = this.dataset
        ...
        this.observeVariantSelector();
        this.updateForTradeOnlyProduct(initialSelectedVariant);
      }
      ...
      syncWithMainProductForm() {
        ...
        window.MinimogEvents.subscribe(`${this.productId}__VARIANT_CHANGE`, async (variant) => {
          if (variant) {
            ...
            this.updateForTradeOnlyProduct(variant.id)
          }
        });
      }
      ...
      observeVariantSelector() {
        const variantSelector = document.querySelector('.m-product-option--dropdown-select')

        variantSelector.addEventListener('change', (e) => {
          this.updateForTradeOnlyProduct(e.target.value)
        })
      }

      updateForTradeOnlyProduct (variantId) {
        const variantIsTradeOnly = this.variantTradeData?.find(({ id }) => id === Number(variantId))?.tradeOnly === true
        const stickyAtcWrapper = document.querySelector('.m-sticky-addtocart')
        const quantityForm = stickyAtcWrapper.querySelector(".m-sticky-addtocart--quantity");
        const atcButton = stickyAtcWrapper.querySelector(".m-add-to-cart");
        const tradeSignInButton = stickyAtcWrapper.querySelector(".trade-sign-in-button");
        const price = stickyAtcWrapper.querySelector(".m-price")
        ...
        if (!this.isTradeCustomer && variantIsTradeOnly) {
          ...
          atcButton.classList.remove('hidden')
